...
def lista_publicaciones(request):
    ...
    perfiles = PerfilUsuario.objects.filter(usuario__in=[p.autor for p in publicaciones])
    
    perfil_dict = {perfil.usuario: perfil for perfil in perfiles}
...
def ver_perfil(request, usuario_id):
    ...
    es_propietario = request.user == perfil

    # Verificar si ya son amigos
    es_amigo = perfil.perfilusuario.amigos.filter(id=request.user.id).exists()

    # Verificar si ya se envió una solicitud de amistad
    solicitud_enviada = Amistad.objects.filter(solicitante=request.user, receptor=perfil, aceptado=False).exists()

    # Verificar si se recibió una solicitud de amistad
    solicitud_recibida = Amistad.objects.filter(solicitante=perfil, receptor=request.user, aceptado=False).exists()
    ...
    return render(request, 'perfil/perfil.html', {
        ...
        'usuario_id': perfil.id  # Asegúrate de pasar el ID del perfil
    })
...
@login_required
def ver_perfil(request, usuario_id):
    perfil_usuario = get_object_or_404(PerfilUsuario, usuario__id=usuario_id)
    
    es_propietario = request.user == perfil_usuario.usuario
    es_amigo = perfil_usuario.usuario.receptor_amistad.filter(solicitante=request.user, aceptado=True).exists()
    solicitud_enviada = Amistad.objects.filter(solicitante=request.user, receptor=perfil_usuario.usuario, aceptado=False).exists()
    solicitud_recibida = Amistad.objects.filter(solicitante=perfil_usuario.usuario, receptor=request.user, aceptado=False).exists()
...
def eliminar_amigo(request, usuario_id):
    try:
        ...
        amistad.delete()
        messages.success(request, 'Amigo eliminado con éxito.')
    except Amistad.DoesNotExist:
        messages.error(request, 'No se encontró la amistad para eliminar.')
    
    return redirect(request.META.get('HTTP_REFERER', 'lista_amigos'))
...
def detalle_grupo(request, grupo_id):
    ...
    amigos = Usuario.objects.exclude(grupos=grupo).exclude(id=request.user.id)
    publicaciones = PublicacionGrupo.objects.filter(grupo=grupo).order_by('-fecha_publicacion')
    
    es_admin = request.user == grupo.creador or request.user.is_superuser

    if request.method == 'POST':
        if 'amigos' in request.POST:  # Añadir amigos
            amigos_ids = request.POST.getlist('amigos')
            grupo.miembros.add(*amigos_ids)
            return redirect('detalle_grupo', grupo_id=grupo.id)
        
        if 'contenido' in request.POST:  # Crear publicación
            contenido = request.POST.get('contenido')
            if contenido:
                ...
                return redirect('detalle_grupo', grupo_id=grupo.id)  # Redirigir al mismo grupo para actualizar las publicaciones
...
def editar_grupo(request, grupo_id):
    ...
    # Verificar si el usuario es el creador o un superusuario
    if request.user != grupo.creador and not request.user.is_superuser:
        return redirect('detalle_grupo', grupo_id=grupo.id)

    if request.method == 'POST':
        form = CrearGrupoForm(request.POST, request.FILES, instance=grupo)
        if form.is_valid():
            form.save()
            return redirect('detalle_grupo', grupo_id=grupo.id)
    else:
